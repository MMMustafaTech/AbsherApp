...
احفظ `challengeId` من الاستجابة. الاستجابة `202 Accepted` تكون متشابهة حتى عند عدم إمكانية التسجيل؛ لا تعتمد عليها لمعرفة إن كان الرقم الوطني موجودًا.
...
الإشعارات حاليًا داخل التطبيق عبر API؛ لا توجد Push Notifications أو SMS حقيقية بعد.
...
لا تعرض رسالة الـBackend التقنية مباشرة للمستخدم؛ اعرض رسالة عربية مناسبة بحسب status code. لا تسجل كلمات المرور أو OTP أو access/refresh tokens داخل logs أو analytics.
