...
interface SpringDataBirthCertificateRequestRepository extends JpaRepository<BirthCertificateRequestJpaEntity,String> {
    @Query("select count(r) > 0 from BirthCertificateRequestJpaEntity r where r.citizenId = :citizenId and r.type = 'BIRTH_CERTIFICATE' and r.openRequestKey = :citizenId") boolean hasOpen(@Param("citizenId") String citizenId);
