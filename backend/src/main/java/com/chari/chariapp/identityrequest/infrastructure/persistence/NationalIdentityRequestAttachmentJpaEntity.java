...
class NationalIdentityRequestAttachmentJpaEntity {
    ...
    private NationalIdentityRequestAttachmentJpaEntity(NationalIdentityRequestAttachment value) { id=value.id().toString(); requestId=value.requestId().toString(); storageKey=value.storageKey(); originalFileName=value.originalFileName(); contentType=value.contentType(); sizeBytes=value.sizeBytes(); uploadedBy=value.uploadedBy().value().toString(); uploadedAt=value.uploadedAt(); }
    ...
    NationalIdentityRequestAttachment toDomain() { return new NationalIdentityRequestAttachment(UUID.fromString(id),UUID.fromString(requestId),storageKey,originalFileName,contentType,sizeBytes,new AccountId(UUID.fromString(uploadedBy)),uploadedAt); }
}
